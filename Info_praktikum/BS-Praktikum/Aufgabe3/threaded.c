...
struct threadParam {                                                                // struct with arguments for thread function
    ...
    int threadCount;                                                                    // amount of desired threads 
...
void* printPrimes(void* args) {                                                         
    ...
    for(int i = data->threadNumber * range; i < limit; i+=range*data->threadCount) {    // iterate through all start points of the threads number ranges 
        for(int j = i; j < i+range; j++) {                                                  // iterate through the number range
            int prime = 1;                                                                      // prime status of the number we check is initially true
            for(int k = 2; k < j; k++) {                                                            
                if(j % k == 0) {                                                                // if the checked number can be divided by any other number (from 2 - checked number) without rest
                    prime = 0;                                                                      // checked number is no prime
                    break;                                                                          // stop checking 
                }
            }
            if(prime && j > 1) {                                                                
                //printf("%d (Thread: %d)\n", j, data->threadNumber);                             // print if it is a prime
...
int main (int argc, char* argv[]) {
    ...
    for(int i = 0; i < threadCnt; i++) {                             // create every thread
        ...
        data[i].threadCount = threadCnt;                                // amount of all calculating threads
